...
def generate_pixel_bold_square_qr():
    qr = qrcode.QRCode(
    ...
    # Clear the background in the center to make the text extremely clear
    ...
    # Render with classic bold squares
    cell_size = 20
    img_size = modules * cell_size
    img = Image.new('RGBA', (img_size, img_size), 'white')
    draw = ImageDraw.Draw(img)
    
    # We will use Navy Blue for a corporate, bold look
    fill_color = (15, 23, 42)
    ...
    for r in range(modules):
        for c in range(modules):
            if matrix[r][c]:
                ...
                # Draw solid square matching the classic ultra-scannable QR standard
                draw.rectangle([x, y, x+cell_size, y+cell_size], fill=fill_color)

    workspace_path = "D:\\WEBSITES BUILT\\Gkk-hire\\gkk_final_square_bold_qr.png"
    ...
    artifacts_path = os.path.join(artifacts_dir, "gkk_final_square_bold_qr_view.png")
    img.save(artifacts_path)
    
    print(f"Artifacts path: {artifacts_path}")

generate_pixel_bold_square_qr()
