(function () {
  ...
  style.textContent = [
    ...
    "#admin-custom-cursor{position:fixed;top:0;left:0;pointer-events:none;z-index:2147483647;display:flex;align-items:center;gap:8px;will-change:transform;opacity:0;visibility:hidden;transform:translate3d(-100px,-100px,0);transition:opacity 0.2s, visibility 0.2s}",
    "#admin-custom-cursor .ac-badge{background:#111;border:1px solid rgba(255,255,255,0.08);border-radius:999px;padding:5px 12px 5px 8px;display:flex;align-items:center;gap:8px;box-shadow:none;white-space:nowrap;max-width:min(72vw,480px);transform-origin:left center;transition:transform 0.15s ease, background 0.2s ease}",
  ...
  var badge = cursor.querySelector(".ac-badge");
  ...
  function update() {
    ...
    nextProbe = now + 40; // Throttle hover detection to 25fps

    var hovered = lastTarget || document.elementFromPoint(lastX, lastY);
    if (!hovered) return;
    ...
    if (badge) {
      badge.style.transform = actionEl ? "scale(1.15)" : "scale(1)";
    }

    var nextLabel = resolveTooltip(hovered);
  ...
  function findActionElement(target) {
    ...
    var selector = "[data-cursor-label], [data-tooltip], button, a, input, textarea, select, summary, [role='button'], [role='link'], [role='tab'], [role='menuitem'], [onclick], [data-action], .clickable, .btn, .button, .cursor-pointer";
  ...
  function resolveTooltip(target) {
    ...
    if (tag === "button") return cleanText(el.textContent) || "Click";
    if (tag === "a") return cleanText(el.textContent) || "Open link";
    if (tag === "input") {
      if (type === "submit" || type === "button" || type === "reset") return cleanText(el.getAttribute("value")) || "Click";
