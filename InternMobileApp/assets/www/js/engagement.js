...
async function loadLeaderboard() {
    ...
    if (typeof SkeletonUI !== 'undefined') {
        container.innerHTML = SkeletonUI.generateTableRows(3, 5); // Mock rows
    } else {
...
async function loadStreakAlert() {
    ...
    try {
        ...
        const avatar = user.avatar_url || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(name);
                    ...
                    <div class="glass-card reward-card" style="overflow: hidden; ${isRedeemed ? 'opacity: 0.7;' : ''}">
                        ${hasImage ? `
                            <div style="margin: -1.5rem -1.5rem 1rem -1.5rem;">
                                <img src="${reward.image_url}" alt="${reward.title}" loading="lazy" 
...
function shareOnWhatsapp(code) {
    const text = `Hey! Join me on GKK-Hire to kickstart your tech career. Use my code *${code}* to get a +50 XP head start! 🚀\n\nApply here: https://gkkintern.site/apply`;
